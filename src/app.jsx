...
export default function App() {
  ...
  // Time & Month Selection (Array of selected month numbers, max 3)
  const [startYear, setStartYear] = useState(2026);
  const [selectedMonths, setSelectedMonths] = useState([10]); // Default: October
  ...
  // Month Toggle Logic (Max 3 Months)
  const toggleMonth = (monthId) => {
    if (selectedMonths.includes(monthId)) {
      if (selectedMonths.length === 1) return; // Prevent deselecting all
  ...
  const getPrice = () => {
    ...
    if (count === 3) return 125; // ₹25 discount
    return 50;
  };
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', maxWidth: '1050px', margin: '0 auto' }}>
      ...
      <form onSubmit={handleGenerate} style={{ background: '#f8fafc', padding: '20px', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '20px' }}>
        ...
        {/* Month Picker Buttons with Tick Options */}
        <div style={{ marginBottom: '15px' }}>
