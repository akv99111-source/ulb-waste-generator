...
import { Building2, Download, Lock, Globe, ShieldCheck, Plus, Trash2, BookOpen, Layers, Sparkles } from 'lucide-react';
...
export default function App() {
  ...
  const [mrfStreamsConfig, setMrfStreamsConfig] = useState(DEFAULT_MRF_STREAMS.map(s => ({ ...s, userWeight: s.defaultWeight })));
  const [mixedStreamsConfig, setMixedStreamsConfig] = useState(DEFAULT_MIXED_STREAMS.map(s => ({ ...s, userWeight: s.defaultWeight })));
  ...
  const updateStreamConfig = (id, field, value) => {
    ...
    setConfig(prev => prev.map(s => s.id === id ? { ...s, [field]: value } : s));
  };

  const addCustomStream = () => {
    const newStream = { id: `custom_${Date.now()}`, label: '', defaultWeight: 0, min: 0, max: 100, isDefault: false, userWeight: 0 };
    if (facilityType === 'MIXED_PLANT') setMixedStreamsConfig([...mixedStreamsConfig, newStream]);
    else setMrfStreamsConfig([...mrfStreamsConfig, newStream]);
  };

  const removeCustomStream = (id) => {
    if (facilityType === 'MIXED_PLANT') setMixedStreamsConfig(mixedStreamsConfig.filter(s => s.id !== id));
    else setMrfStreamsConfig(mrfStreamsConfig.filter(s => s.id !== id));
  };
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <div style={{ background: '#f0f9ff', border: '1px solid #7dd3fc', padding: '12px 16px', borderRadius: '8px', marginBottom: '16px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: '13px', color: '#0c4a6e', lineHeight: '1.5' }}>
              {lang === 'hi' 
                ? '1. प्लांट प्रकार चुनें। 2. विवरण भरें और महीने चुनें। 3. लॉग-बुक जनरेट करें। 4. सुरक्षित भुगतान करके एक्सेल डाउनलोड करें।' 
                : '1. Select facility type. 2. Enter details & select months. 3. Generate dataset. 4. Pay securely to download full Excel file.'}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleGenerate} style={{ background: '#fff', padding: '20px', borderRadius: '8px', border: '1px solid #cbd5e1', marginBottom: '20px' }}>
          ...
          <div style={{ marginBottom: '14px', display: 'flex', gap: '15px', alignItems: 'center', fontSize: '14px', flexWrap: 'wrap', background: '#f8fafc', padding: '10px', borderRadius: '6px', border: '1px solid #e2e8f0' }}>
            ...
            <label style={{ cursor: 'pointer' }}><input type="radio" value="MRF" checked={facilityType === 'MRF'} onChange={() => { setFacilityType('MRF'); setGeneratedMonthlyData(null); }} /> {lang === 'hi' ? 'एमआरएफ (MRF) (₹100–150)' : 'MRF Centre (₹100–150/mo)'}</label>
            ...
            <div style={{ marginTop: '10px', background: isAdvancedMode ? '#fffbeb' : '#f8fafc', padding: '12px', borderRadius: '6px', border: '1px solid #cbd5e1', marginBottom: '14px' }}>
              ...
              {isAdvancedMode && (
                <>
                  ...
                  <button type="button" onClick={addCustomStream} style={{ marginTop: '10px', padding: '6px 12px', background: '#e2e8f0', color: '#334155', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>
                    <Plus size={14} style={{ verticalAlign: 'middle' }} /> Add Custom Fraction
                  </button>
                </>
              )}
            </div>
          ...
          <button type="submit" disabled={generateDisabled} style={{ width: '100%', padding: '12px', background: generateDisabled ? '#94a3b8' : '#059669', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}>
        ...
        {generatedMonthlyData && (
          <div ref={resultsRef} style={{ background: '#fff', padding: '15px', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
              <strong>{name} Dataset Preview</strong>
              {isPaid && <button onClick={downloadExcel} style={{ padding: '6px 12px', background: '#0f172a', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}><Download size={13} /> Export Excel</button>}
            </div>
                  ...
                  {visibleRows.map((r, idx) => (
                    <tr key={idx} style={{ borderBottom: '1px solid #e2e8f0' }}>
                      ...
                      {facilityType === 'ULB' ? (
                        ...
                        <>{generatedConfig.streams.map(s => <td key={s.id}>{formatVal(r.streams[s.id])}</td>)}<td><strong>{formatVal(r.total)}</strong></td></>
                      )}
                    </tr>
                  ))}
                </tbody>
            ...
            {!isPaid && (
              <div style={{ border: '2px dashed #059669', background: '#ecfdf5', padding: '15px', textAlign: 'center', marginTop: '12px', borderRadius: '6px' }}>
                ...
                <button onClick={handlePayment} disabled={isProcessing} style={{ padding: '10px 20px', background: '#059669', color: '#fff', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer', marginTop: '8px' }}>
                  {isProcessing ? 'Processing...' : `Pay ₹${pricing.total} & Download File`}
                </button>
              </div>
            )}
          </div>
        ...
        <footer style={{ marginTop: '40px', paddingTop: '20px', borderTop: '1px solid #cbd5e1', textAlign: 'center', fontSize: '12px', color: '#64748b' }}>
          ...
          <p style={{ margin: 0 }}>© {new Date().getFullYear()} Consilience Research Foundation / SWM Engine. All Rights Reserved.</p>
        </footer>
            ...
            <div style={{ background: '#fff', padding: '24px', borderRadius: '8px', maxWidth: '600px', width: '100%', maxHeight: '80vh', overflowY: 'auto' }}>
              ...
              {activePolicyModal === 'terms' && (
                <div>
                  ...
                  <p style={{ fontSize: '12px' }}>This tool provides engineered estimations for municipal waste management. Data generated is intended for planning, monitoring, and audit preparation.</p>
                </div>
              )}
              {activePolicyModal === 'refunds' && (
                <div>
                  <h2 style={{ fontSize: '18px', fontWeight: 'bold' }}>Refunds & Cancellations</h2>
                  <p style={{ fontSize: '12px' }}>Excel datasets are delivered digitally instantly. In cases of duplicate payment debits without dataset unlocking, refunds are credited within 5-7 working days.</p>
                </div>
              )}
              {activePolicyModal === 'pricing' && (
                <div>
                  ...
                  <ul style={{ fontSize: '12px', lineHeight: '1.8' }}>
                    ...
                    <li>MRF Logbook (Base): ₹100 / Month</li>
                    <li>Advanced MRF Custom Logbook: ₹150 / Month</li>
                    <li>Mixed Waste Plant Logbook: ₹200 / Month</li>
                  </ul>
                </div>
              )}
